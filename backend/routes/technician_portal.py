...
async def technician_ai_assist(request: Request, data: AIAssistRequest, user: dict = Depends(get_current_technician)):
    ...
    try:
        ...
        system_message = f"""You are Battwheels EVFI™ — India's AI-powered Electric Vehicle Failure Intelligence engine. You are helping {technician_name} with EV repairs and diagnostics.
...
3. RESPONSE FORMAT — ALWAYS use:
   ...
   ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
   ⚡ SAFETY PRECAUTIONS — [Mandatory safety steps]
   📋 DIAGNOSTIC STEPS — Step 1: [Action] — Expected: [Result]
   🔧 PROBABLE ROOT CAUSES (ranked)
   🛒 PARTS THAT MAY BE NEEDED — with approx ₹ prices
   ⏱️ ESTIMATED TIME: [X] minutes
   ⚠️ This guidance is generated by Battwheels EVFI™ AI and should be verified by a qualified EV technician.

4. LANGUAGE: Default Hinglish for steps, English for technical terms.

5. NEVER: guess DTC codes, recommend opening sealed battery packs without model-specific procedure, bypass safety interlocks, say "I think/believe", give generic advice without model reference."""
        ...
        # Post-process: inject safety warning
        ticket_data = {
            "description": data.query,
            "issue_type": data.category or "",
            "category": data.category or "",
            "vehicle_make": data.context.get("vehicle_make", "") if data.context else "",
            "vehicle_model": data.context.get("vehicle_model", "") if data.context else "",
        }
        from services.ai_guidance_service import inject_safety_warning, classify_efi_response
