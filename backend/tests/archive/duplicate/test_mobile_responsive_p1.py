...
======================================================
Tests:
  - GET /api/v1/platform/audit-status (platform admin endpoint)
  - GET /api/v1/organizations/me/members (was crashing with KeyError: membership_id)
  - POST /api/v1/auth/login for both org_admin and platform_admin
"""
...
PLATFORM_ADMIN_PASS = "DevTest@123"
...
class TestPlatformAuditStatus:
    """Test GET /api/v1/platform/audit-status endpoint"""

    def test_audit_status_returns_200(self, platform_admin_token):
        """Verify audit-status returns 200 for platform admin"""
        headers = {"Authorization": f"Bearer {platform_admin_token}"}
        res = requests.get(f"{BASE_URL}/api/v1/platform/audit-status", headers=headers)
        assert res.status_code == 200, f"Expected 200, got {res.status_code}: {res.text}"
        print(f"PASS: GET /api/v1/platform/audit-status returned 200")
    ...
    def test_audit_status_denied_without_auth(self):
        ...
        res = requests.get(f"{BASE_URL}/api/v1/platform/audit-status")
...
class TestOrganizationMembers:
    """Test GET /api/v1/organizations/me/members - was crashing with KeyError: membership_id"""
    ...
    def test_members_returns_200(self, org_admin_token):
        ...
        res = requests.get(f"{BASE_URL}/api/v1/organizations/me/members", headers=headers)
        assert res.status_code == 200, f"Expected 200, got {res.status_code}: {res.text}"
        print(f"PASS: GET /api/v1/organizations/me/members returned 200")
...
class TestPlatformMetrics:
    """Test GET /api/v1/platform/metrics endpoint"""

    def test_metrics_returns_200(self, platform_admin_token):
        """Verify metrics endpoint returns 200"""
        headers = {"Authorization": f"Bearer {platform_admin_token}"}
        res = requests.get(f"{BASE_URL}/api/v1/platform/metrics", headers=headers)
        assert res.status_code == 200, f"Expected 200, got {res.status_code}: {res.text}"
        data = res.json()
