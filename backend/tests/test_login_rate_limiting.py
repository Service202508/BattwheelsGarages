...
def _clear_all_test_attempts(base_url):
    """Clear all rate limit entries for test emails before each test."""
    import pymongo
    import os
...
class TestLoginRateLimiting:
    ...
    def test_rate_limit_first_5_attempts_allowed(self, _base):
    ...
    def test_rate_limit_6th_attempt_returns_429(self, _base):
        ...
        time.sleep(0.3)  # Ensure all DB writes complete under load
        resp = requests.post(_base, json={"email": email, "password": "wrong_6"})
        assert "Too many login attempts" in resp.json().get("detail", "")

    def test_rate_limit_successful_login_resets_counter(self, _base):
    ...
    def test_rate_limit_different_emails_tracked_independently(self, _base):
