  ...
  return (
    <div className="space-y-6" data-testid="customers-page">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold text-bw-white tracking-tight">Customers</h1>
          <p className="text-bw-white/[0.45] text-sm mt-1">{total} customers in database</p>
        </div>
        <Dialog open={showAddDialog} onOpenChange={setShowAddDialog}>
          <DialogTrigger asChild>
            <Button className="bg-bw-volt hover:bg-bw-volt-hover text-bw-black font-bold" data-testid="add-customer-btn">
          ...
          <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
            ...
            <div className="flex justify-end gap-2">
              ...
              <Button onClick={handleAddCustomer} className="bg-bw-volt hover:bg-bw-volt-hover text-bw-black font-bold">
                Add Customer
              </Button>
            </div>
          </DialogContent>
      ...
      {/* Customer List */}
      {loading ? (
        <div className="text-center py-12 text-bw-white/[0.45]">Loading customers...</div>
      ) : customers.length === 0 ? (
        <Card>
        ...
        <div className="grid gap-4">
          {customers.map((customer) => (
            <Card key={customer.customer_id} className="border border-white/[0.07] hover:border-bw-volt/20 transition-colors" data-testid={`customer-card-${customer.customer_id}`}>
              <CardContent className="p-4">
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    <div className="flex items-center gap-3 mb-2">
                      <h3 className="font-semibold text-bw-white">{customer.display_name}</h3>
                    ...
                    {customer.gstin && (
                      <div className="mt-2 text-xs text-bw-white/[0.45]">
                  ...
                  <div className="flex items-center gap-4">
                    <div className="text-right">
                      ...
                      <p className={`font-semibold ${customer.outstanding_balance > 0 ? 'text-bw-orange' : 'text-green-600'}`}>
                      ...
                      <DialogContent className="max-w-lg">
                        ...
                        <div className="space-y-4 py-4">
                          <div className="grid grid-cols-2 gap-4 text-sm">
                            <div>
                              <p className="text-bw-white/[0.45]">Customer ID</p>
                              <p className="font-medium">{customer.customer_id}</p>
                            </div>
                            <div>
                              <p className="text-bw-white/[0.45]">Company</p>
                              <p className="font-medium">{customer.company_name || '-'}</p>
                            </div>
                            <div>
                              <p className="text-bw-white/[0.45]">Phone</p>
                              <p className="font-medium">{customer.phone || '-'}</p>
                            </div>
                            <div>
                              <p className="text-bw-white/[0.45]">Email</p>
                              <p className="font-medium">{customer.email || '-'}</p>
                            </div>
                            <div>
                              <p className="text-bw-white/[0.45]">GSTIN</p>
                              <p className="font-medium">{customer.gstin || '-'}</p>
                            </div>
                            <div>
                              <p className="text-bw-white/[0.45]">Payment Terms</p>
                              <p className="font-medium">{customer.payment_terms || 15} days</p>
                            </div>
                            <div className="col-span-2">
                              <p className="text-bw-white/[0.45]">Billing Address</p>
                            ...
                            <div>
                              <p className="text-bw-white/[0.45]">Outstanding Balance</p>
                              <p className={`font-semibold text-lg ${customer.outstanding_balance > 0 ? 'text-bw-orange' : 'text-green-600'}`}>
                                ₹{(customer.outstanding_balance || 0).toLocaleString('en-IN')}
                              </p>
                            </div>
                          </div>
