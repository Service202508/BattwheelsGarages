/**
 * EstimatesTable — Summary cards, conversion funnel, quick actions,
 * Estimates table tab, and Ticket Estimates tab.
 * All rendered as fragments so the parent Tabs wrapper works correctly.
 */
...
import {
  ...
  CheckCircle, ArrowRightLeft, Ticket, Send
} from "lucide-react";
...
/* Summary Cards + Funnel + Quick Actions (rendered above Tabs) */
export function EstimatesSummary({ state, handlers }) {
  const { summary, funnel, selectedIds, statusFilter } = state;
  const {
    setStatusFilter, fetchEstimates, handleExport,
    setShowImportDialog, setShowBulkActionDialog, setShowCustomFieldsDialog,
    setShowTemplateDialog, setShowPreferencesDialog,
    fetchCustomFields, fetchPdfTemplates, fetchPreferences,
  } = handlers;
    ...
    <>
      ...
      <div className="flex flex-wrap justify-between items-center gap-2">
        <div className="flex gap-2">
          <Button variant="outline" size="sm" onClick={() => handleExport("csv")} className="gap-2 bg-transparent border-[rgba(255,255,255,0.13)] text-[rgba(244,246,240,0.70)] hover:border-[rgba(200,255,0,0.30)] hover:text-[#F4F6F0]" data-testid="export-csv-btn"><FileDown className="h-4 w-4" /> Export CSV</Button>
          <Button variant="outline" size="sm" onClick={() => setShowImportDialog(true)} className="gap-2 bg-transparent border-[rgba(255,255,255,0.13)] text-[rgba(244,246,240,0.70)] hover:border-[rgba(200,255,0,0.30)] hover:text-[#F4F6F0]" data-testid="import-btn"><FileUp className="h-4 w-4" /> Import</Button>
          {selectedIds.length > 0 && (
            <Button variant="outline" size="sm" onClick={() => setShowBulkActionDialog(true)} className="gap-2 bg-[rgba(59,158,255,0.08)] border-[rgba(59,158,255,0.25)] text-[#3B9EFF] hover:bg-[rgba(59,158,255,0.15)]" data-testid="bulk-action-btn"><ListChecks className="h-4 w-4" /> Bulk Actions ({selectedIds.length})</Button>
          )}
        </div>
        <div className="flex gap-2">
          <Button variant="outline" size="sm" onClick={() => { fetchCustomFields(); setShowCustomFieldsDialog(true); }} className="gap-2 bg-transparent border-[rgba(255,255,255,0.13)] text-[rgba(244,246,240,0.70)]" data-testid="custom-fields-btn"><Edit className="h-4 w-4" /> Custom Fields</Button>
          <Button variant="outline" size="sm" onClick={() => { fetchPdfTemplates(); setShowTemplateDialog(true); }} className="gap-2 bg-transparent border-[rgba(255,255,255,0.13)] text-[rgba(244,246,240,0.70)]" data-testid="templates-btn"><LayoutTemplate className="h-4 w-4" /> Templates</Button>
          <Button variant="outline" size="sm" onClick={() => { fetchPreferences(); setShowPreferencesDialog(true); }} className="gap-2 bg-transparent border-[rgba(255,255,255,0.13)] text-[rgba(244,246,240,0.70)]" data-testid="preferences-btn"><Settings className="h-4 w-4" /> Preferences</Button>
        </div>
      </div>
        ...
        <Card className="bg-[#111820] border border-[rgba(255,255,255,0.07)]">
          ...
          <CardContent className="pt-4">
            <div className="flex items-center justify-between text-xs">
              <div className="text-center"><p className="font-mono text-[10px] uppercase tracking-[0.12em] text-[rgba(244,246,240,0.45)]">Created</p><p className="text-xl font-bold text-[#F4F6F0]">{funnel.total_created}</p></div>
              <ChevronRight className="h-4 w-4 text-[rgba(255,255,255,0.15)]" />
              <div className="text-center"><p className="font-mono text-[10px] uppercase tracking-[0.12em] text-[rgba(244,246,240,0.45)]">Sent</p><p className="text-xl font-bold text-[#F4F6F0]">{funnel.sent_to_customer}</p><p className="text-[#3B9EFF] text-xs">{funnel.send_rate}%</p></div>
              <ChevronRight className="h-4 w-4 text-[rgba(255,255,255,0.15)]" />
              <div className="text-center"><p className="font-mono text-[10px] uppercase tracking-[0.12em] text-[rgba(244,246,240,0.45)]">Accepted</p><p className="text-xl font-bold text-[#F4F6F0]">{funnel.accepted}</p><p className="text-[#C8FF00] text-xs">{funnel.acceptance_rate}%</p></div>
              <ChevronRight className="h-4 w-4 text-[rgba(255,255,255,0.15)]" />
              <div className="text-center"><p className="font-mono text-[10px] uppercase tracking-[0.12em] text-[rgba(244,246,240,0.45)]">Converted</p><p className="text-xl font-bold text-[#F4F6F0]">{funnel.converted}</p><p className="text-[#8B5CF6] text-xs">{funnel.conversion_rate}%</p></div>
            </div>
          </CardContent>
        </Card>
      )}
...
/* Estimates Tab Content (TabsContent value="estimates") */
export function EstimatesTabContent({ state, handlers }) {
  const { estimates, loading, search, setSearch, statusFilter, selectedIds } = state;
  const {
    setStatusFilter, fetchEstimates, fetchEstimateDetail, handleClone,
    toggleSelectAll, toggleSelect, setActiveTab,
  } = handlers;
  ...
  return (
    <TabsContent value="estimates" className="space-y-4">
      <div className="flex flex-col sm:flex-row gap-4 justify-between">
        ...
        <Button onClick={() => setActiveTab("create")} className="bg-[#C8FF00] hover:bg-[#d4ff1a] text-[#080C0F] font-bold gap-2 rounded-[3px]" data-testid="new-estimate-btn"><Plus className="h-4 w-4" /> New Estimate</Button>
      </div>
        ...
        <div className="bg-[#0D1317] border border-[rgba(255,255,255,0.07)] rounded">
          <ResponsiveTable>
            <thead className="bg-[#111820] border-b border-[rgba(255,255,255,0.07)]">
              <tr>
                <th className="px-2 py-3 text-center w-10"><input type="checkbox" checked={selectedIds.length === estimates.length && estimates.length > 0} onChange={toggleSelectAll} className="h-4 w-4 rounded border-[rgba(255,255,255,0.2)] bg-transparent" onClick={(e) => e.stopPropagation()} /></th>
            ...
            <tbody>
              {estimates.map(est => (
                <tr key={est.estimate_id} className={`border-b border-[rgba(255,255,255,0.04)] hover:bg-[rgba(200,255,0,0.03)] cursor-pointer transition-colors ${selectedIds.includes(est.estimate_id) ? 'bg-[rgba(59,158,255,0.08)]' : ''}`} onClick={() => fetchEstimateDetail(est.estimate_id)} data-testid={`estimate-row-${est.estimate_id}`}>
                  <td className="px-2 py-3 text-center" onClick={(e) => e.stopPropagation()}><input type="checkbox" checked={selectedIds.includes(est.estimate_id)} onChange={() => toggleSelect(est.estimate_id)} className="h-4 w-4 rounded border-[rgba(255,255,255,0.2)] bg-transparent" /></td>
...
/* Ticket Estimates Tab Content */
export function TicketEstimatesTabContent({ state, handlers }) {
  const { ticketEstimates } = state;
  const { fetchEstimateDetail, handleOpenEdit } = handlers;
                  ...
                  {ticketEstimates.map((est) => (
                    <tr key={est.estimate_id} className="border-b hover:bg-[rgba(59,158,255,0.04)] cursor-pointer" onClick={() => fetchEstimateDetail(est.estimate_id)}>
                      <td className="px-4 py-3">
                        <a href={`/tickets?id=${est.ticket_id}`} className="text-[#3B9EFF] hover:underline font-mono text-sm" onClick={(e) => e.stopPropagation()}>{est.ticket_id?.slice(0, 12)}...</a>
                      </td>
                      ...
                      <td className="px-4 py-3 text-center"><EstimateStatusBadge status={est.status} /></td>
                      <td className="px-4 py-3 text-right" onClick={(e) => e.stopPropagation()}>
                        <Button size="sm" variant="ghost" onClick={() => window.open(`/tickets?id=${est.ticket_id}`, '_blank')} className="text-[#3B9EFF] hover:text-[#5BB0FF]">Open Ticket</Button>
                        <Button size="sm" variant="ghost" onClick={() => handleOpenEdit({ ...est, is_ticket_estimate: true })}><Eye className="h-4 w-4 mr-1" /> View</Button>
                      </td>
                    </tr>
                  ))}
                </tbody>
