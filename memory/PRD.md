...
## Changelog
- **Feb 2026**: Final Pre-Deployment Audit — Data sanitisation (30 test orgs purged, 0 orphaned records), 17/17 flows pass. Fixed: (1) CRITICAL: POST /api/payments KeyError 'total_amount' — now uses .get() with grand_total fallback; (2) MAJOR: invoice['invoice_number'] direct dict access replaced with .get(); (3) MEDIUM: /tickets mobile horizontal overflow fixed (overflow-x-hidden + min-w-0). Slug fixed: battwheels-default → battwheels-garages. organizations.slug index added. DB final state: 1 org, 13 users, 2 tickets, 1 invoice, 7 journal entries.
- **Feb 2026**: Platform Admin Leads tab — Added Leads tab to /platform-admin. Backend: 3 new endpoints in `platform_admin.py` (`GET /platform/leads`, `PATCH /platform/leads/{id}/status`, `PATCH /platform/leads/{id}/notes`), all behind `require_platform_admin`. Frontend: `LeadRow` component with inline status dropdown (New/Called/Qualified/Closed-Won/Closed-Lost, color-coded), expandable notes textarea (saves on blur), summary row (Total/New This Week/Qualified/Conversion Rate).
- **Feb 2026**: Homepage CTA redesign — Fixed Book Demo → Login misdirection. Nav now has: Login (plain text link → /login), BOOK DEMO (opens lead capture modal), FREE TRIAL (→ /register). Hero "Book Enterprise Demo" and bottom CTA "Request Enterprise Consultation" both open the same modal. BookDemoModal collects Name, Workshop, City, Phone, Vehicles/month. Backend `/api/book-demo` stores lead in `demo_requests` collection, emails sales@battwheels.com via Resend. Whitelisted in all 4 auth middleware files.
- **Feb 2026**: Public ticket multi-tenancy redesign — replaced hardcoded default-org DB lookup with `get_org_from_request()` subdomain extraction (workshopname.battwheels.com → slug lookup) with `X-Organization-Slug` header fallback. Fixed cross-tenant data leak in `lookup_ticket` (org-scoped query). Fixed `get_public_ticket` sub-queries (estimate, invoice, payments, activities all scoped by `ticket.organization_id`). Added null-org guard. Updated `PublicTicketForm.jsx` and `TrackTicket.jsx` to pass `X-Organization-Slug` header. Added Customer Intake Form URL to `OrganizationSettings.jsx`.
- **Feb 2026**: Coding standards & tenant isolation guards — Created `/app/CODING_STANDARDS.md` (3 prevention rules with correct/incorrect code examples for Patterns A/B/C). Added "Prevention Rules Checklist" section with 9 spot-check commands to `/app/PRE_DEPLOYMENT_AUDIT.md`. Added `# TENANT GUARD` single-line comment to 9 route files: tickets, invoices_enhanced, journal_entries, finance_dashboard, hr, inventory, contacts_enhanced, efi_intelligence, reports.
...
- **Feb 2026**: FINAL CERTIFICATION — 35/35 tests pass across INFRA, AUTH, FINANCE, OPERATIONS, HR, SECURITY, CONTACT, EFI, PLATFORM, DB, DEPS. react-router-dom upgraded 7.11.0→7.13.1 (XSS CVEs fixed). yarn resolutions added to force patched transitive deps. Remaining known-acceptable: jsonpath (build-time, no fix), ecdsa (no upstream fix). App certified production-ready. All 4 match the `SaaSLanding.jsx` design system exactly (#080C0F bg, #C8FF00 volt green, monospace font, grain overlay). Footer links in `SaaSLanding.jsx` updated from `href="#"` to React Router navigation. Content is authentic to the platform's actual features, Indian law (PDPB, CGST Act), and real technical architecture.
