...
| Test | Result | Evidence |
|------|--------|----------|
| **MN8.01 [CRITICAL] Sentry backend** | ✅ PASS | `SENTRY_DSN=https://8b734d...` configured in `.env`. Sentry initialized in `server.py` on startup. |
| **MN8.02 [CRITICAL] Sentry frontend** | ✅ PASS | `REACT_APP_SENTRY_DSN=https://ae17014...` in `frontend/.env`. React ErrorBoundary wraps app in `index.js`. |
...
- **Remediation:** In `server.py`, replace `_cors_origins_raw = "*"` with your specific domains: `allow_origins=["https://production-hardened-2.preview.emergentagent.com", "https://app.battwheels.com"]`. Set `CORS_ORIGINS` env var in production.
